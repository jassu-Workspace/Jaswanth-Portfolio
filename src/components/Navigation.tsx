  ...
  return (
    <>
      ...
      >
        ...
        <button
          className="md:hidden text-[#00d0ff]"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed top-[64px] left-0 w-full z-40 premium-glass px-8 py-6 flex flex-col gap-4 md:hidden"
          >
            {navLinks.map((link) => (
              <a
                key={link.href}
                ...
                className="text-gray-300 hover:text-[#00d0ff] font-medium transition-colors text-base"
              >
                {link.label}
              </a>
            ))}
            <a
              href={`mailto:${CONTACT_EMAIL}`}
              className="mt-2 bg-[#00d0ff] text-[#0a191f] px-6 py-3 rounded-lg font-bold text-center hover:brightness-110 transition-all"
            >
              Contact Me
            </a>
          </motion.div>
